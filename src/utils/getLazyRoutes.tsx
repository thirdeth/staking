...
export default function getLazyRoutes(routes: RoutesProps, loggedIn: boolean) {
  const routesArr = formatRoutesToArr(routes).map(({ dirName, root: { path, id, isProtected }, ...nested }) => {
    const Component = lazy(() => import(`pages/${dirName}`).then((module) => ({ default: module[dirName] })));
