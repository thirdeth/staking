...
import { getAddLiquidityTime } from '../actions';
import actionTypes from '../actionTypes';
import { updateIdoState } from '../reducer';

export function* getAddLiquidityTimeSaga({
  type,
  payload: { web3Provider, idoIncrement },
}: ReturnType<typeof getAddLiquidityTime>) {
  yield* put(request(type));
  const { chainType }: UserState = yield select(userSelector.getUser);
  const { currentIdo }: IdoState = yield select(idoSelector.getIdo);
  ...
  try {
    ...
    const liquidityAddedTime = yield* call(idoFarmeContract.methods.addLiqTime(idoIncrement).call);

    yield* put(
      updateIdoState({
        currentIdo: { ...currentIdo, liquidityAddedTime },
      }),
    );
...
export default function* listener() {
  yield takeLatest(actionTypes.GET_INVESTMENTS_INFO, getAddLiquidityTimeSaga);
}
