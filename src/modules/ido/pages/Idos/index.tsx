    ...
    (page: number) => {
      ...
      dispatch(
        getIdoList({
          ...
          status: (searchParams.get(PARAMS.status) as IdoStatus) || IdoStatus.pending,
  ...
  return (
    <Box sx={{ overflowX: 'hidden' }}>
      <StageBar
        ...
        idoStatus={(searchParams.get(PARAMS.status) as IdoStatus) || IdoStatus.pending}
        onChangeFilter={handleChangePublicFilter}
        onChangeStage={handleChangeIdoStatus}
      />

      <Grid pt={2} container spacing={2}>
        <Grid item xs={12} display={{ xs: 'none', sm: 'none', md: 'block' }}>
          <CardsHeader status={searchParams.get(PARAMS.status) || (IdoStatus.pending as string)} />
        </Grid>
        {idos.length &&
          idos.map((idoData) => (
